...
var Color ColorOutput
var frontMap map[string]int
var backMap map[string]int

func init() {
	Color = ColorOutput{frontColor: FrontRed, backColor: BackBlack, mode: ModeDefault}
...
func (c ColorOutput) Printf(structural interface{}) {
	c.frontColor = FrontYellow
	jsonStr, _ := json.Marshal(structural)
	fmt.Println(fmt.Sprintf("%c[%d;%d;%dm%s%c[0m", 0x1B, c.mode, c.backColor, c.frontColor, jsonStr, 0x1B))
}

// 设置前景色
func (c ColorOutput) Front(color string) ColorOutput {
...
// 设置背景色
func (c ColorOutput) Back(color string) ColorOutput {
	color = strings.ToLower(color)
	co, ok := backMap[color]
	if ok {
		c.backColor = co
...
// 设置模式
func (c ColorOutput) Mode(mode int) ColorOutput {
